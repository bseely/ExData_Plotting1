## Script to make Plot 4 and save to a PNG file
...
## Make 2x2 chart array, export to PNG
dev.new(width=480, height=480, units="px")
par(mfrow=c(2,2))
## 1 of 4
plot(working_data$timeStamp, working_data$Global_active_power, type="l", xlab="", ylab="Global Active Power (kilowatts)")
## 2 of 4
plot(working_data$timeStamp, working_data$Voltage, type="l", xlab="datetime", ylab="Voltage")
## 3 of 4
plot(working_data$timeStamp, working_data$Sub_metering_1, type="l", xlab="", ylab="Energy Sub Metering")
points(working_data$timeStamp, working_data$Sub_metering_2, type="l", col="red")
points(working_data$timeStamp, working_data$Sub_metering_3, type="l", col="blue")
legend(x="topright", legend=c("Sub_metering_1    ","Sub_metering_2    ","Sub_metering_3    "), col=c("black", "red", "blue"), bty="n",lty=1)
## 4 of 4
plot(working_data$timeStamp, working_data$Global_reactive_power, type="l", xlab="datetime", ylab="Global Reactive Power")
dev.copy(png, file="./figure/plot4.png",width = 480, height = 480,units = "px", pointsize = 12, bg = "white", res = 72,
         restoreConsole = TRUE)
dev.off()
